...
WHERE i.ssn=(SELECT ssn FROM person p
WHERE license_id ="202298" );--310,000, Miranda

SELECT annual_income  FROM income i
WHERE i.ssn=(SELECT ssn FROM person p
WHERE license_id ="291182" );--does not find data...

SELECT annual_income  FROM income i
WHERE i.ssn=(SELECT ssn FROM person p
WHERE license_id ="918773" ); --278,000 Red
...
SELECT id  FROM person p
WHERE p.license_id =(SELECT id  FROM drivers_license dl
WHERE car_make  LIKE 'Tesla' AND car_model ="Model S" AND hair_color ="red" AND gender ="female"
);--persons id (suspect) 99716

SELECT name  FROM person p
WHERE id ='99716';

 -- crime organizer-> Miranda Priestly
