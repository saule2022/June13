...
import CassandraExample3.{getClusterSession, rawQuery, runQuery}
...
object Day15CassandraCRUD extends App {
  ...
  println("Latvian:")
  val LvResults = session.execute("SELECT * FROM vs_keyspace.users_by_country WHERE country='LV'")
  LvResults.forEach(row => println(row))
  println("Lithuanian:")
  val LtResults = session.execute("SELECT * FROM vs_keyspace.users_by_country WHERE country='LT'")
  LtResults.forEach(row => println(row))
